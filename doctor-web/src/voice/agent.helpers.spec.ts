...
describe('agent.helpers', () => {
  describe('searchDoctorsInFirestore', () => {
    it('should query Firestore and return formatted results', async () => {
      ...
      expect(mockCollection.where).toHaveBeenCalledWith('expertiseList', 'array-contains', specialty);
  ...
  describe('persistVoiceChatHistory', () => {
    it('should skip non-message items to avoid undefined role errors', async () => {
      ...
      // 3. Call helper
      await persistVoiceChatHistory(mockDb, 'user123', messages, 'Cardiology', []);
      ...
      expect(mockSet).not.toHaveBeenCalledWith(expect.objectContaining({ role: undefined }), expect.anything());
    });
  });
});
