...
- **CI / GitHub Actions & Deployment**: The project utilizes an automated pipeline sequence covering: **Lint Checks**, **Testing Checks** (via Jest), and **Security Checks** (npm audit). Passing all checks automatically triggers the **Render Deploy**. When updating dependencies, always run `npm install --legacy-peer-deps` locally and push both `package.json` and `package-lock.json` to prevent `npm ci` from throwing `ERESOLVE` mismatches in these actions.
- **Pre-commit Measures**: We use Husky to enforce quality locally before pushing. It runs formatting (Prettier), lint rules (ESLint), and code assertions (Jest). A commit failing these checks will abruptly abort the push.
